...
export default function AsstDeanReviewsPage() {
  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      try {
        ...
        const response = await fetch(`/api/reviews/asst-dean${termParam}`)
  ...
  const filteredTeachers = teachers.filter(teacher => {
    ...
    return hodReview && hodReview.submitted
  })
  ...
  return (
    <RoleGuard allowedRoles={['ASST_DEAN']}>
      <DashboardLayout title="Assistant Dean Reviews">
          ...
          <div className="bg-white shadow rounded-lg">
            <div className="px-4 py-5 sm:p-6">
              <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">
                Teachers with Completed HOD Reviews
              </h3>
              ...
              {filteredTeachers.length > 0 ? (
                              ...
                              {selectedTerm !== 'ALL' ? (
                                <>
                                  {getReviewStatus(teacher, selectedTerm).status !== 'HOD_PENDING' && (
                                    <Link
                                      href={`/dashboard/asst-dean/reviews/${teacher.id}/${selectedTerm}`}
                                ...
                                <>
                                  {getReviewStatus(teacher, 'START').status !== 'HOD_PENDING' && (
                                    <Link
                                      href={`/dashboard/asst-dean/reviews/${teacher.id}/START`}
                                  ...
                                  {getReviewStatus(teacher, 'END').status !== 'HOD_PENDING' && (
                                    <Link
                                      href={`/dashboard/asst-dean/reviews/${teacher.id}/END`}
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-400">
                    Teachers will appear here once their HOD reviews are completed
                  </p>
                </div>
              )}
            </div>
