...
export async function GET(request: NextRequest) {
  try {
    ...
    // Prepare response data with all scores
    const evaluationData = {
      hodComment: hodReview?.comments || null,
      hodScore: hodScores?.overallRating || hodScores?.score || hodScores?.rating || null, // Try multiple possible field names
    ...
    console.error('Error fetching evaluation report data:', error)
